...
class Subject extends Admin_Controller
{
    ...
    public function getByClassSection()
    {
        ...
        if (!empty($classID)) {
            ...
            if (is_array($getClassTeacher) && count($getClassTeacher) < 1) {
                $this->db->select('timetable_class.subject_id');
                ...
                $query = $this->db->select('id,subject_id')->where(array('class_id' => $classID, 'section_id' => $sectionID))->get('timetable_class');
            }
            if ($query->num_rows() != 0) {
